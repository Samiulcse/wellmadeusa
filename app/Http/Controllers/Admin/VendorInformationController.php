...
class VendorInformationController extends Controller
{
    ...
    public function sizeChartPost(Request $request) {
        $user = Auth::user();
        $user->load('vendor');
        $user->vendor->return_policy = $request->description;
        $user->vendor->save();
    }
    public function savereturn_info(Request $request) {
        $vendor = MetaVendor::where('id', 1)->first();
        $vendor->return_policy = $request->description;
        $vendor->save();
        return back();
    }
    ...
    public function orderNoticePost(Request $request) {
        $user = Auth::user();
        $user->load('vendor');
        $user->vendor->order_notice = $request->order_notice_editor;
        $user->vendor->save();

    }

    public function admin_shipping_post(Request $request) {
        $user = Auth::user();
        $user->load('vendor');
