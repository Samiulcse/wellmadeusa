...
class AdminNewOrderController extends Controller
{
    public function itemListAll(Request $request) {
        ...
        $companyData = MetaBuyer::where('user_id', $sessionUser )->first();

        return view('admin.dashboard.new_order.index', compact('userData','companyData','TotalactiveItems','activeItems', 'defaultCategories', 'vendorCategories' , 'customers' , 'current_customer' , 'cart_items' , 'selected_items_list', 'appends'))
    ...
    public function nameSearch(Request $request){
        ...
        if($search == ''){
            $users = DB::table('users')
                ->select('users.id','users.first_name','meta_buyers.company_name')
                ->join('meta_buyers','meta_buyers.user_id','=','users.id')
...
//                ->where('users.deleted_at', '=', NULL)
        ...
        foreach($users as $user){
            $response[] = array("value"=>$user->company_name,"label"=>$user->first_name,"user_id"=>$user->id);
        }
    ...
    public function companySearch(Request $request){
        ...
        $response = array();
        foreach($users as $user){
            $response[] = array("value"=>$user->first_name,"label"=>$user->company_name,"user_id"=>$user->id);
        }
