...
@section('content')
    <form class="form-horizontal" method="post" action="{{ route('admin_edit_item_post', ['item' => $item->id]) }}" id="form" enctype="multipart/form-data">
        @csrf
        <div class="text_left m15">
            ...
            <div class="display_inline mr_0 float_right">
                <a href="{{ route('admin_clone_item', ['item' => $item->id]) }}" class="ly_btn btn_blue" style="color: white">Clone</a>
                @if ($prevItemId)
                    <a href="{{ route('admin_edit_item', ['item' => $prevItemId]) }}" class="ly_btn btn_blue text_center min_width_100p">Prev</a>
                @endif
                @if ($nextItemId)
                    <a href="{{ route('admin_edit_item', ['item' => $nextItemId]) }}" class="ly_btn btn_blue text_center min_width_100p">Next</a>
                @endif
                                ...
                                @if (in_array($color->id, $previous_color_ids))
                                    ...
                                    <tr class="inv_{{ $color->id }}">
                                        ...
                                        <td>
                                            ...
                                            <div class="display_t_cell_50">
                                                <div class="datepicker_wrapper">
                                                    <input class="text-center form_global available_on_inv" name="inv[{{$invIndex}}][available_on]" <?php if($availability_inv == 1 || $availability_inv == 3){echo "disabled";} ?> value="{{$inv_av}}" autocomplete="off" placeholder="Available On">
                                                </div>
                                            </div>
                                        </td>
                                    </tr>
    ...
    <script>
        $(function () {
            ...
            $('body').on('change','.available_on_inv', function(){
                ...
                var available_on = $('#available_on').html();
                if(available_on === '' || setDate < available_on){
                    $('#available_on').val($(this).val());
                    $('#available_on_date').html($(this).val());
                }
            });
