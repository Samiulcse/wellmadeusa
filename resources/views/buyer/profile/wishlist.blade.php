...
<div class="myaccount_from common_form">
    <div class="row">
        @if ( ! $items->isEmpty() )
            @foreach ( $items as $product )
            <div class="com-sm-6 col-6 col-md-4">
                <div class="save_list">
                    <img src="{{ asset('/' . $product->images[0]['image_path']) }}" alt="Avatar" class="save_list_image img-fluid">
                    <div class="middle">
                        <a  href="{{ route('product_single_page', $product->slug) }}" class="wishlist_add_bag">Add to Bag</a>
                        <button type="button" class="remove_wishlist" id="remove_{{$product->id}}" title="Remove item" data-id="{{ $product->id }}">Remove</button>
                    </div>
                </div>
